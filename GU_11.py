...
class UroflowmetryApp:
    ...
    def connect_serial(self):
        ...
        try:
            ...
            logging.info(f"Connected to COM 5")
            time.sleep(2)
        except serial.SerialException as e:
            logging.error(f"Failed to connect to COM 5: {str(e)}")
            Messagebox.show_error("Serial Error", f"Failed to connect to COM5: {str(e)}")
            self.root.destroy()
    ...
    def create_home_ui(self):
        ...
        buttons = [
            ...
            ("Save", self.save_data),
            ("Restart", self.restart_UI),
            ("Shutdown", self.shutdown_UI),
        ]
    ...
    def create_history_ui(self):
        ...
        buttons = [
            ...
            ("Refresh", self.refresh_history_ui)
        ]
        for text, command in buttons:
    ...
    def refresh_history_ui(self):
        ...
        Renumbers the id_nomor to ensure a continuous sequence starting from 1,
        based on the oldest data (created_at).
        ...
        try:
            ...
            with sqlite3.connect('V3_UROFLOWMETRY_DATABASE.db') as conn:
                cursor = conn.cursor()
                # Fetch all patient records ordered by created_at
                cursor.execute("SELECT * FROM patients ORDER BY created_at DESC")
                rows = cursor.fetchall()

                # Renumber id_nomor starting from 1
                for index, row in enumerate(rows, start=1):
                    self.history_treeview.insert("", "end", values=(
                        index,  # New id_nomor starting from 1
                        row[4],  # patient_id_num
                        f"{row[1]} {row[2]}",  # Name
                        row[10]  # created_at
                    ))

            Messagebox.show_info("Success", "History refreshed successfully")
        except sqlite3.OperationalError as e:
            logging.error(f"Database Error: {str(e)}")
            Messagebox.show_error(f"Database Error: {str(e)}", f"Table 'patients' does not exist: {str(e)}")
        except Exception as e:
            logging.error(f"Error refreshing history: {str(e)}")
            Messagebox.show_error(f"Error refreshing history: {str(e)}", f"Failed to refresh history: {str(e)}")
    ...
    def save_data(self):
        ...
        def submit():
            try:
                ...
                if not (
                        first and last and age and patient_id and gender and doctor_first and doctor_last and hospital and address):
                ...
                # Generate PDF
                self.generate_pdf(patient_id, first, last, gender, hospital, address, doctor_first, doctor_last, self.y2_data)
    ...
    def generate_pdf(self, patient_id, first, last, gender, hospital, address, doctor_first, doctor_last, y2_data):
        """
        ...
        try:
            ...
            total_volume = sum(y2_data) if y2_data else 0
            ...
            data = [
                ...
                ["Total Volume", f"{total_volume:.2f} mL"]
            ]
    ...
    def restart_UI(self):
        """
        Restarts the application by closing the current instance and relaunching it.
        Ensures proper resource cleanup before restarting.
        """
        try:
            self.close_resources()  # Ensure all resources are closed
            logging.info("Restarting the application...")
            os.execv(sys.executable, ['python'] + sys.argv)
        except Exception as e:
            logging.error(f"Error restarting the application: {str(e)}")
            Messagebox.show_error("Restart Error", f"Failed to restart the application: {str(e)}")

    def shutdown_UI(self):
        """
        Shuts down the application by closing all resources and destroying the main window.
        """
        try:
            self.close_resources()  # Ensure all resources are closed
            ...
            logging.error(f"Error shutting down the application: {str(e)}")
            Messagebox.show_error("Shutdown Error", f"Failed to shut down the application: {str(e)}")
